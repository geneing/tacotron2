...
class LPCNetLoader(torch.utils.data.Dataset):
    """
        1) loads audio,text pairs
        2) normalizes text and converts them to sequences of one-hot vectors
        3) loads lpcnet input features from file.
    """
    def __init__(self, audiopaths_and_text, hparams):
        self.audiopaths_and_text = load_filepaths_and_text(audiopaths_and_text)
        self.text_cleaners = hparams.text_cleaners
    ...
    def get_lpc_text_pair(self, audiopath_and_text):
        # separate filename and text
        audiopath, text = audiopath_and_text[0], audiopath_and_text[1]
        text = self.get_text(text)
        lpcfeatures = self.get_lpcnet_features(audiopath)
        return (text, lpcfeatures)

    def get_lpcnet_features(self, filename):
    ...
    def __getitem__(self, index):
        return self.get_lpc_text_pair(self.audiopaths_and_text[index])
